  ...
  return (
    <div className="min-h-screen bg-[#071326]">
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex items-center justify-between animate-fade-in-up">
          <div className="flex items-center gap-4">
            <Link
              href="/shop"
              className="flex items-center gap-2 rounded-[14px] bg-white/5 px-4 py-2 transition-colors hover:bg-[#1E1E1E]"
            >
              <Home className="h-5 w-5" />
              <span className="font-medium">Trang Chủ</span>
            </Link>

            <h1 className="flex items-center gap-3 text-3xl font-bold text-white/90">
          ...
          <a
            ...
            className="flex items-center gap-2 rounded-[14px] bg-[#2F9BE6] px-4 py-2 font-medium text-white/90 transition-colors hover:bg-[#49B6FF]"
          >
          ...
          <>
                    ...
                    <div className="mb-3 space-y-2">
                      {currentItems.map((item, itemIndex) =>
                        isEditing ? (
                          <div
                            key={itemIndex}
                            className="space-y-2 rounded-[14px] border border-white/10 bg-[#071326]/60 p-2"
                          >
                            <input
                              value={item.name}
                              onChange={(e) => updateEditingItem(itemIndex, "name", e.target.value)}
                              className="w-full rounded border border-white/10 bg-white/5 px-2 py-1 text-sm text-white/90 outline-none"
                            />
                            <input
                              type="number"
                              step="0.01"
                    ...
                    <div className="flex items-center justify-between border-t border-white/10 pt-3">
                      ...
                      {isAdmin && (
                        <div className="flex gap-1.5">
                          <label className="inline-flex cursor-pointer items-center gap-1 rounded bg-[#1E1E1E] px-2 py-1.5 text-xs text-white/90 transition-colors hover:bg-[#2F9BE6]" title="Add proof image">
                          ...
                          {isEditing ? (
                            <>
                              <button
                                ...
                                disabled={saving}
                                className="rounded bg-[#3DDC84] p-1.5 text-white/90 transition-colors hover:bg-[#3DDC84]/90 disabled:opacity-50"
                                title="Lưu"
                              >
                              ...
                              <button
                                type="button"
                                onClick={cancelEdit}
                                className="rounded bg-[#1E1E1E] p-1.5 text-white/90 transition-colors hover:bg-[#1E1E1E]"
                                title="Hủy"
                              >
                                <X className="h-4 w-4" />
                              </button>
                            ...
                            <>
                              <button
                                ...
                                className="rounded bg-[#1E1E1E] p-1.5 text-white/90 transition-colors hover:bg-[#49B6FF]"
                                title="Sửa"
                              >
                              ...
                              <button
                                ...
                                className="rounded bg-[#1E1E1E] p-1.5 text-white/90 transition-colors hover:bg-[#FF4D4F]/90"
                                title="Xóa"
                              >
                                <Trash2 className="h-4 w-4" />
                              </button>
            ...
            <div className="flex items-center justify-center gap-4 animate-fade-in">
              <button
                ...
                className="flex items-center gap-2 rounded-[14px] bg-white/5 px-4 py-2 font-medium text-white/90 transition-colors hover:bg-[#1E1E1E] disabled:cursor-not-allowed disabled:opacity-50"
              >
      ...
      {lightbox && activeImages.length > 0 && (
        ...
        >
          <button
            className="absolute right-4 top-4 rounded-[14px] bg-white/5 p-2 transition-colors hover:bg-[#1E1E1E]"
            onClick={closeLightbox}
          >
