      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex items-center justify-between animate-fade-in-up">
          <div className="flex items-center gap-4">
            ...
            <h1 className="flex items-center gap-3 text-3xl font-bold text-[#071326]/90">
            ...
            <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {proofs.map((proof, idx) => {
                ...
                return (
                  <div
                    key={proof.id}
                    className="animate-vouch-entrance rounded-[16px] border border-white/50 bg-white/60 p-5 backdrop-blur-sm transition-all hover:border-white/50"
                    ...
                    {proof.imageUrls.length > 0 ? (
                      <div className={`mb-4 grid gap-2 ${getGalleryGridClass(proof.imageUrls.length)}`}>
                        {proof.imageUrls.slice(0, 2).map((url, imageIndex) => (
                          <div key={imageIndex} className={`group relative overflow-hidden rounded-[14px] bg-white/60 ${getImageSpanClass(proof.imageUrls.length, imageIndex)}`}>
                            <button
                              type="button"
                            ...
                            {isAdmin && (
                              <div className="absolute right-2 top-2 flex gap-1 opacity-100 sm:opacity-0 sm:transition-opacity sm:group-hover:opacity-100">
                                ...
                                <button
                                  ...
                                  className="inline-flex h-8 w-8 items-center justify-center rounded bg-white/60 text-[#071326]/90 hover:bg-[#FF4D4F]"
                                  title="Delete image"
                                >
                      ...
                      <div className="mb-4 flex h-32 w-full items-center justify-center rounded-[14px] bg-white/60">
                        ...
                        isEditing ? (
                          ...
                          >
                            <input
                              ...
                              className="w-full rounded border border-white/50 bg-white/60 px-2 py-1 text-sm text-[#071326]/90 outline-none"
                            />
                            <input
                              type="number"
                              step="0.01"
                          ...
                          <div key={itemIndex} className="flex justify-between gap-3 text-sm">
                            <div>
                              ...
                              <p className="mt-1 text-xs text-slate-600">Qty: {item.deliveredLabel}</p>
                            </div>
      ...
      {lightbox && activeImages.length > 0 && (
        ...
        >
          ...
          {activeImages.length > 1 && (
            <button
              className="absolute left-4 top-1/2 -translate-y-1/2 rounded-full bg-white/60 p-3"
              onClick={(e) => {
                e.stopPropagation()
                prevImage()
              }}
            >
              <ChevronLeft className="h-8 w-8 text-[#071326]/90" />
            </button>
          )}
          ...
          {activeImages.length > 1 && (
            <button
              className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full bg-white/60 p-3"
              onClick={(e) => {
                e.stopPropagation()
                nextImage()
              }}
            >
              <ChevronRight className="h-8 w-8 text-[#071326]/90" />
            </button>
          )}

          {activeImages.length > 1 && (
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full bg-white/60 px-4 py-2 text-sm font-medium text-[#071326]/90">
