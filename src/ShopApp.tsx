...
import { ApiError, api, assetUrl, backendUrl, dateTime, depositMethod, depositStatus, money, orderStatus, uploadImage } from './api'
import type { AdminChat, AdminOrderChat, BalanceLog, ChatMessage, Deposit, GameCategory, Item, Notification, Order, OrderItem, Review, Settings, User } from './types'
...
const discordCheckoutStorageKey = 'pending_discord_checkout'
...
function saveDiscordCheckout(payload: DiscordCheckoutPayload) {
  window.sessionStorage.setItem(discordCheckoutStorageKey, JSON.stringify(payload))
}

function loadDiscordCheckout() {
  try {
    const raw = window.sessionStorage.getItem(discordCheckoutStorageKey)
    if (!raw) return null
    const parsed = JSON.parse(raw)
    if (!parsed || !parsed.kind || !parsed.page) return null
    return parsed as DiscordCheckoutPayload
  } catch {
    return null
  }
}

function clearDiscordCheckout() {
...
function discordLinkUrl() {
  return backendUrl(`/discord/link?return_to=${encodeURIComponent('/?discord_linked=1')}`)
}
...
function ShopApp({ initialPageOverride }: ShopAppProps = {}) {
  const [page, setPage] = useState<Page>(() => initialPageOverride || initialPage())
  const [routeId, setRouteId] = useState<string>('')
  const [user, setUser] = useState<User | null>(null)
  const [booting, setBooting] = useState(true)
  const [bootProgress] = useState(0)
  ...
  const [discordCheckoutResumeToken, setDiscordCheckoutResumeToken] = useState(0)
  void setBootLabel
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        ...
        {page === 'item' && <ItemDetail {...context} slug={routeId} resumeToken={discordCheckoutResumeToken} />}
        ...
        {page === 'cart' && <CartPage user={user} cart={cart} setCart={setCart} go={go} setUser={setUser} setNotice={showNotice} resumeToken={discordCheckoutResumeToken} />}
  ...
  const submitPurchase = useCallback(async (event?: FormEvent, override?: { quantity: number; robloxUsername: string; customerNote: string }) => {
    ...
    try {
      const data = await api<{ order: Order }>('/orders/buy', {
        method: 'POST',
        body: JSON.stringify({ itemId: item.id, ...purchase }),
      })
    ...
    <>
      ...
      <section className="page-section detail-layout">
        ...
        <div className="detail-card">
          <button className="ghost" onClick={() => go('items')}>Quay l?i danh s?ch item</button>
          <h1>{item.name}</h1>
          ...
          <div className="info-box">
            <h3>H??ng d?n nh?n item</h3>
            <p>Nh?p ??ng Roblox Username, gi? online ho?c theo d?i th?ng b?o ?? admin li?n h? giao item.</p>
            <h3>L?u ? tr??c khi mua</h3>
            <p>Shop ch? b?n item Sailor Piece, kh?ng h?i m?t kh?u Roblox v? kh?ng b?n nick/account.</p>
          </div>
          <form className="form-card" onSubmit={submitPurchase}>
            ...
            <label>S? l??ng mua<input required min={1} max={item.stock} type="number" placeholder="Nh?p s? l??ng" value={numberInputValue(form.quantity)} onChange={(event) => setForm({ ...form, quantity: numberInputNext(event.target.value) })} /></label>
            <textarea placeholder="Ghi ch? cho shop" value={form.customerNote} onChange={(event) => setForm({ ...form, customerNote: event.target.value })} />
            <button type="button" onClick={() => addToCart(item, safeQuantity(form.quantity))}>Th?m v?o gi? h?ng</button>
            <button className="primary large" disabled={item.stock < 1}>Mua ngay b?ng s? d?</button>
          </form>
          <div className="review-list">
            <h2>??nh gi? ?? duy?t</h2>
...
function CartPage({ user, cart, setCart, go, setUser, setNotice, resumeToken = 0 }: { user: User | null; cart: CartItem[]; setCart: (cart: CartItem[]) => void; go: (page: Page, id?: string) => void; setUser: (user: User | null) => void; setNotice: (message: string) => void; resumeToken?: number }) {
  ...
  const submitCheckout = useCallback(async (event?: FormEvent, override?: { robloxUsername: string; customerNote: string; items: Array<{ itemId: number; quantity: number }> }) => {
    ...
    if (!cart.length && !override?.items?.length) {
      setNotice('Gi? h?ng ?ang tr?ng.')
      return
    }
    const purchase = override || {
      robloxUsername: form.robloxUsername,
  ...
  return (
    <>
      ...
      <section className="page-section two-col">
        <div className="panel">
          <h1>Gi? h?ng</h1>
          {!cart.length && <p className="muted">Gi? h?ng ?ang tr?ng.</p>}
          {cart.map((entry) => (
            <div className="cart-row" key={entry.item.id}>
              ...
              <button onClick={() => setCart(cart.filter((item) => item.item.id !== entry.item.id))}>X?a</button>
            </div>
          ))}
          <h2>T?ng: {money(total)}</h2>
        </div>
        <form className="panel" onSubmit={submitCheckout}>
          <h2>Thanh to?n gi? h?ng</h2>
          <input required placeholder="Roblox Username" value={form.robloxUsername} onChange={(event) => setForm({ ...form, robloxUsername: event.target.value })} />
          <textarea placeholder="Ghi ch? cho shop" value={form.customerNote} onChange={(event) => setForm({ ...form, customerNote: event.target.value })} />
          <LoadingButton className="primary large" loading={submitting} disabled={!cart.length}>Mua t?t c? b?ng s? d?</LoadingButton>
          <button type="button" onClick={() => go('items')}>Ti?p t?c ch?n item</button>
        </form>
      </section>
    </>
  )
  ...
  return (
    <section className="page-section">
      ...
      <section className="profile-actions">
        <div className="panel">
          <h2>Discord</h2>
          {summary.user.discord_id ? (
            <>
              <p>Đã liên kết với <strong>{summary.user.discord_username || summary.user.discord_id}</strong>.</p>
              <p className="muted">Liên kết lúc {dateTime(summary.user.discord_linked_at)}</p>
            </>
          ) : (
            <>
              <p>Liên kết Discord để checkout tạo ticket tự động.</p>
...
function ChatPage({ user, go, setNotice, locationId = '' }: { user: User | null; go: (page: Page, id?: string) => void; setNotice: (message: string) => void; locationId?: string }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (locationId) {
        await api('/orders/' + locationId + '/chat', { method: 'POST', body: JSON.stringify({ message, image_url: imageUrl }) })
      } else {
  ...
  if (!user) return <section className="page-section"><h1>Vui l?ng ??ng nh?p ?? chat v?i admin.</h1><button className="primary" onClick={() => go('login')}>??ng nh?p</button></section>
  const visibleMessages = locationId ? orderDetail?.messages || [] : messages
  return (
    <section className="page-section">
      <div className="panel chat-panel">
        {locationId && orderDetail && (
          <div className="order-header">
            <h1>??n {orderDetail.order.order_code}</h1>
            <p>Tr?ng th?i: <strong>{orderStatus[orderDetail.order.status] || orderDetail.order.status}</strong></p>
            <p>Ticket Discord: <strong>{orderDetail.order.discord_ticket_status || 'not_created'}</strong></p>
            {orderDetail.order.discord_ticket_url && <p><a href={orderDetail.order.discord_ticket_url} target="_blank" rel="noreferrer">M? ticket Discord</a></p>}
            {Array.isArray(orderDetail.items) && orderDetail.items.length > 0 && <div className="order-summary-list">{orderDetail.items.map((entry) => <div key={entry.id}><strong>{entry.item_name}</strong><span>x{entry.quantity}</span></div>)}</div>}
          </div>
        )}
        <h1>{locationId ? 'Ticket nh?n h?ng' : 'Chat'}</h1>
        <div className="chat-box">
          {visibleMessages.length === 0 && <p className="muted">Ch?a c? tin nh?n n?o.</p>}
          {visibleMessages.map((item) => <ChatBubble key={item.id} item={item} mine={item.sender_id === user.id} />)}
        </div>
        <form className="chat-form" onSubmit={submit}>
          <input placeholder="Nh?p tin nh?n c?n h? tr?" value={message} onChange={(event) => setMessage(event.target.value)} />
          <label className="upload-button">?nh<input type="file" accept="image/*" onChange={uploadChatImage} /></label>
          <button className="primary" disabled={!message && !imageUrl}>G?i</button>
        </form>
        {imageUrl && <img className="chat-image-preview" src={assetUrl(imageUrl)} alt="?nh chu?n b? g?i" />}
      </div>
    </section>
  )
}
